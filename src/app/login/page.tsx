...
export default function Login() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Note: the AuthContext will handle updating `user`, but since we want to redirect
            // immediately with the role, we should theoretically wait for context or just fetch it here.
            // For simplicity, we can let AuthContext catch it and the effect at the top of the file will redirect.
            // However, relying on the listener is sometimes slow, so let's push to the context's redirect logic.
            // We just let the effect handle the routing.
            // (Alternative is fetching the doc here, but avoiding double-fetch is better).
        } catch (err: any) {
            console.error(err);
            ...
            <div className="w-full max-w-md bg-surface-dark p-8 md:p-10 rounded-2xl border border-white/10 shadow-tech">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <p className="text-gray-400 mt-2 text-center text-sm">
                        Bienvenido de vuelta a Corredores de Oriente
                    </p>
                </div>
                ...
                <button
                    onClick={async () => {
                        try {
                            ...
                            // Check if document exists first, create it if not
